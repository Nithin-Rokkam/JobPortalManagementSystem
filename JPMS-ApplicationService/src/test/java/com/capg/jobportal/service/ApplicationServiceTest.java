...
class ApplicationServiceTest {
    ...
    @Test
    void deleteApplication_owner_success() {
        when(applicationRepository.findById(1L)).thenReturn(Optional.of(testApplication));
        when(jobServiceClient.getJobById(eq(200L), anyString(), anyString()))
                .thenReturn(testJobResponse);
        doNothing().when(applicationRepository).delete(testApplication);

        applicationService.deleteApplication(1L, 50L);
    ...
    void deleteApplication_notFound_throwsException() {
        ...
        assertThrows(ResourceNotFoundException.class,
                () -> applicationService.deleteApplication(99L, 50L));
    }

    @Test
    void deleteApplication_notOwner_throwsForbidden() {
        when(applicationRepository.findById(1L)).thenReturn(Optional.of(testApplication));
        when(jobServiceClient.getJobById(eq(200L), anyString(), anyString()))
                .thenReturn(testJobResponse);

        assertThrows(ForbiddenException.class,
                () -> applicationService.deleteApplication(1L, 999L));
    }

    @Test
    void deleteApplication_jobNull_throwsForbidden() {
        when(applicationRepository.findById(1L)).thenReturn(Optional.of(testApplication));
        when(jobServiceClient.getJobById(eq(200L), anyString(), anyString()))
                .thenReturn(null);
    ...
    @Test
    void updateApplicationStatus_shortlistedToSelected() {
        testApplication.setStatus(ApplicationStatus.SHORTLISTED);
        StatusUpdateRequest request = new StatusUpdateRequest();
        request.setNewStatus(ApplicationStatus.SELECTED);

        when(applicationRepository.findById(1L)).thenReturn(Optional.of(testApplication));
        when(jobServiceClient.getJobById(eq(200L), anyString(), anyString()))
                .thenReturn(testJobResponse);
        when(applicationRepository.save(any(Application.class))).thenReturn(testApplication);
        when(authServiceClient.getUserInfo(anyLong())).thenReturn(new UserInfoResponse());
    ...
    @Test
    void updateApplicationStatus_selectedToRejected() {
        testApplication.setStatus(ApplicationStatus.SELECTED);
        StatusUpdateRequest request = new StatusUpdateRequest();
        request.setNewStatus(ApplicationStatus.REJECTED);
