...
public class GatewayJwtFilter implements GlobalFilter, Ordered {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        logger.debug("Incoming request — Path: {} Method: {}", path, method);

        // Allow CORS preflight requests (OPTIONS) to pass through without authentication
        if ("OPTIONS".equalsIgnoreCase(method)) {
            logger.debug("Allowing CORS preflight (OPTIONS) for path: {}", path);
        ...
        logger.debug("Authenticated request — Path: {} UserId: {} Role: {}", path, userId, role);
