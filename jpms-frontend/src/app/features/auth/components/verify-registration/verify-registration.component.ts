...
export class VerifyRegistrationComponent implements OnInit, OnDestroy {
    ...
    onVerify(): void {
        ...
        // TEST BYPASS: OTP "000000" — call the API which accepts it on the backend.
        // If the API call fails for any reason (e.g. old container), do NOT count
        // it as a failed attempt and show a clear message instead.
        ...
        this.authApi.verifyRegistrationOtp(this.email, otp).subscribe({
            ...
            error: (err: any) => {
                ...
                // If bypass OTP was used but failed, the backend container may not
                // have the bypass yet — show a clear message and do NOT decrement attempts
                if (isBypass) {
                    this.errorMsg = 'Bypass not available yet. Please use the real OTP from your email.';
                    this.clearBoxes();
                    this.focus(0);
                    return;
                }

                // Normal failure — decrement attempts
                this.attemptsLeft--;
                this.clearBoxes();
                this.focus(0);
