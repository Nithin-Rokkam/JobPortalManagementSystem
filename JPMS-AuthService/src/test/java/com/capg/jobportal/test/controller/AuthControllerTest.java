...
class AuthControllerTest {
    ...
    @Test
    void verifyRegistration_returnsOk() throws Exception {
        doNothing().when(authService).verifyRegistrationOtp(anyString(), anyString());

        mockMvc.perform(post("/api/auth/verify-registration")
                .contentType(MediaType.APPLICATION_JSON)
                .content("{\"email\":\"nithin@gmail.com\",\"otp\":\"482931\"}"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.message").value("Email verified successfully. You can now log in."));
    }

    @Test
    void resendRegistrationOtp_returnsOk() throws Exception {
        doNothing().when(authService).resendRegistrationOtp(anyString());

        mockMvc.perform(post("/api/auth/resend-registration-otp")
                .contentType(MediaType.APPLICATION_JSON)
                .content("{\"email\":\"nithin@gmail.com\"}"))
                .andExpect(status().isOk())
    ...
    @Test
    void updateCompanyName_returnsOk() throws Exception {
        doNothing().when(authService).updateCompanyName(eq(1L), anyString());

        mockMvc.perform(put("/api/auth/profile/company")
                .contentType(MediaType.APPLICATION_JSON)
